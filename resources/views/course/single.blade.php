                ...
                <ol role="list" class="flex items-center space-x-0.5">
                    ...
                    <li>
                        <div class="flex items-center">
                            ...
                            <div class="-m-1">
                                <a href="{{route('course-single', $couresSingle->slug)}}"
                                   class="p-1 ml-0.5 text-base font-medium text-gray-400 rounded-md focus:outline-none focus:ring-2 focus:text-gray-900 focus:ring-gray-900 hover:text-gray-700">{{$couresSingle->name}} </a>
                            </div>
                        </div>
                    </li>
                </ol>
            ...
            <div class="flex mt-8">
                <div class="mt-2 w-2/3 border-gray-100 border-r pr-10 pb-0 border-b-0">
                    ...
                    <div class="flex items-center gap-3 mt-5">
                        ...
                        <a href="#reviews" class="text-sm text-gray-500">{{ count($couresSingle->reviews)}}
                            Review{{count($couresSingle->reviews) > 1 ? 's' :  ''}} </a>
                    </div>
                    ...
                    <div class="rounded-sm mt-8 shadow-sm bg-white">
                        ...
                        <div class="p-6">
                            <div class="text-base space-y-6 font-normal text-gray-700 line-clamp-20">
                                <p p-2>{{$couresSingle->description}}</p>
                                <hr>
                                <h1 class="text-4xl font-bold">Reviews</h1>
                                @if(count($couresSingle->reviews) > 0)
                                    @foreach($couresSingle->reviews as $review) @endforeach
                                        <h4 class="mt-3">{{$review->user->name}}</h4>
                                    <h2 class="mt-4 pt-4 text-lg font-bold text-gray-900">{{$review->comment}}</h2>
                ...
                <div class="w-1/3 pl-10 pt-0">
                    <div class="mt-2 mb-4 space-y-3">
                        <a href="#" target="_blank"
                           class="flex items-center justify-center w-full px-4 py-3 text-base font-bold text-center text-white transition-all duration-200 bg-red-500 border-2 border-transparent rounded focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-900 hover:bg-white hover:text-black hover:border-b-2 hover:border-red-500">
                            Get the
                            {{$couresSingle->type == 0 ? 'Book' : 'video' }} &nbsp;
                    ...
                    <div class="overflow-hidden bg-white border border-gray-200 rounded-sm shadow-sm">
                        <div class="space-y-4 p-6">
                            ...
                            @foreach($couresSingle->authors as $author)
                                <div class="space-y-4 pb-2">
                                    ...
                                    <ul role="list" class="flex space-x-5 justify-center">
                                        ...
                                        @endif
                                        @if(!empty($author->githubLink))
                                            <li>
                                                <a href="{{$author->githubLink}}"
                                                   class="text-gray-400 hover:text-gray-500" target="_blank">
                                ...
                                <div class="px-4 py-5 sm:p-6">
                                    ...
                                    <div class="mt-8 flex flex-wrap gap-2.5">
                                        @foreach($couresSingle->topics as $topics)
                                            <a href="{{route('topics', $topics->slug)}}"
                                               class="relative inline-flex items-center rounded-full border border-gray-300 px-3 py-1.5 hover:bg-gray-100">
                                                ...
                                                <div
                                                    class="ml-3.5 text-sm font-medium text-gray-900">{{$topics->name}}</div>
                                            </a>
                                        @endforeach
                                    </div>
                                </div>
